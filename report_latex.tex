...
% --- For clickable TOC links (optional) ---
\usepackage{hyperref}
...
\textbf{KHOA CÔNG NGHỆ THÔNG TIN}}\par
\vspace{1cm}

\includegraphics[width=0.3\textwidth]{logoITSGU.png}\par\vspace{1cm}

{\Huge\bfseries Báo cáo Kiểm thử phần mềm\par } 
\vspace{1cm}
...
\noindent {\large\textbf{THÀNH VIÊN:}}\par\vspace{0.3cm}
{\large
Nguyễn Mạnh Đức      3122410088\par\vspace{0.2cm}
Phạm Quốc Bảo        3122410022\par\vspace{0.2cm}
Nguyễn Tường Huy     3123410128\par\vspace{0.2cm}
Nguyễn Duy Bảo       3122410018\par\vspace{0.2cm}
Vũ Mai Bằng          3123410032\par
}
\end{minipage}
\end{center}
...
Dự án là một website cơ bản được xây dựng nhằm thực hành quy trình phát triển và kiểm thử phần mềm.  
Website bao gồm các chức năng chính như đăng ký, đăng nhập người dùng và hiển thị danh sách sản phẩm.  
Mục tiêu của dự án là giúp sinh viên hiểu rõ quy trình từ thiết kế, lập trình đến kiểm thử, đồng thời áp dụng các công cụ hiện đại trong phát triển web.

\vspace{0.5cm}

\noindent\textbf{Công nghệ sử dụng:}

\begin{itemize}
    \item \textbf{Frontend:} ReactJS, HTML, CSS, JavaScript, Axios
    \item \textbf{Backend:} Spring Boot, Spring Web, Spring Data JPA
    \item \textbf{Database:} MySQL 
    \item \textbf{Công cụ kiểm thử:} Postman, Jest / React Testing Library, JUnit, Mockito
    \item \textbf{Quản lý mã nguồn và CI/CD:} GitHub, GitHub Actions
\end{itemize}

\section{Test cases chi tiết}
...
\begin{enumerate}
    \item \textbf{Login:} 10 test scenarios + 5 test cases chi tiết
    \item \textbf{Product Management:} 10 test scenarios + 5 test cases chi tiết
\end{enumerate}

Tổng cộng 20 scenarios covering happy paths, negative tests, boundary tests, edge cases, và security tests.

\subsection{Các vấn đề phát hiện}

\subsubsection{Vấn đề nghiêm trọng (Critical):}

\begin{enumerate}
    \item \textbf{UPDATE/DELETE không kiểm tra ownership}
    \begin{itemize}
        \item User A có thể sửa/xóa product của User B
        \item Cần thêm authorization check ngay
    \end{itemize}
    
    \item \textbf{Session management không an toàn}
    \begin{itemize}
        \item Lưu user data trực tiếp vào localStorage
        \item Nên chuyển sang JWT token
    \end{itemize}
\end{enumerate}

\subsubsection{Vấn đề quan trọng (High):}

\begin{enumerate}
    \setcounter{enumi}{2}
    \item \textbf{Thiếu pagination}
    \begin{itemize}
        \item GET /api/products/\{userId\} trả về toàn bộ products
        \item Có thể gây performance issue với nhiều products
    \end{itemize}
    
    \item \textbf{Validation chưa đầy đủ}
    \begin{itemize}
        \item Frontend và backend validation không đồng bộ
        \item Cần unified validation rules
    \end{itemize}
\end{enumerate}

\subsection{Khuyến nghị cải tiến}

\subsubsection{Ưu tiên 1 (Ngay lập tức):}
\begin{itemize}
    \item Thêm ownership check cho UPDATE/DELETE endpoints
    \item Implement JWT authentication thay localStorage
    \item Fix security vulnerabilities
\end{itemize}

\subsubsection{Ưu tiên 2 (Ngắn hạn):}
\begin{itemize}
    \item Thêm pagination cho product list
    \item Unified validation (frontend + backend)
    \item Thêm rate limiting để prevent brute force
\end{itemize}

\subsubsection{Ưu tiên 3 (Dài hạn):}
\begin{itemize}
    \item Thêm Price và Category fields cho Product
    \item Implement product search và filter
    \item Add image upload cho products
    \item Thêm audit log cho security tracking
\end{itemize}
...
\hline
\textbf{Module} & \textbf{Scenarios} & \textbf{Test Cases} & \textbf{Coverage} \\
\hline
Login & 10 & 5 & Happy path + Security + Validation \\
\hline
Product & 10 & 5 & CRUD + Security + Boundary \\
\hline
\textbf{Total} & \textbf{20} & \textbf{10} & \textbf{Comprehensive} \\
\hline
\end{tabular}
